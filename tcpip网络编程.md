...
> 可以联系四次挥手。
>
> 四次挥手就没有使用延迟确认，因为服务器的ACK报文和FIN报文是分开发送的，而不是延迟后一起发送。
...
| :------: | :----------------------------------------------------------: |
|   = 0    |                         对端关闭连接                         |
|   > 0    |           成功发送（send）或收取（recv） n 个字节            |
| < 0(-1)  | 出错 OR 被信号中断 OR 对端 TCP 窗口太小数据发不出去（send）或者当前网卡缓冲区已无数据可收（recv） |

我们来逐一介绍下这三种情况：

- **返回值等于 0**

    对端关闭了连接，我们这端也关闭连接即可，这是实际开发时最常见的处理逻辑。

- **返回值小于 0**

    对于阻塞 socket，则一定表示出错。对于非阻塞 socket，则不一定，这里列一个表格说明：(Linux下)

    |      |            返回值和错误码            |           send 函数            |         recv 函数          |
    | ---- | :----------------------------------: | :----------------------------: | :------------------------: |
    | 1    | 返回 -1，错误码 EWOUDBLOCK 或 EAGAIN | TCP 窗口太小，数据暂时发不出去 | 当前内核缓冲区中无可读数据 |
    | 2    |        返回 -1，错误码 EINTR         |      被信号中断，需要重试      |    被信号中断，需要重试    |
    | 3    |    返回 -1，错误码不是上述 1 和 2    |              出错              |            出错            |

- **返回值大于 0**

    表示发送了或接收了多少字节，==需要注意的是，在这种情形下，我们一定要判断下 send 函数的返回值是不是我们期望发送的字节数长度，而不是简单判断其返回值大于 0。==
...
> 因为**UDP通信没有连接的概念**，所以我们**每次读写数据都需要 获取发送端/接收端 的socket地址和长度。**
>
